...
export async function GET(
  ...
  try {
    ...
    console.error('[Admin] 获取助手详情错误:', error)
    return NextResponse.json(
      { error: '服务器错误' },
      { status: 500 }
    )
...
export async function PATCH(
  ...
  try {
    ...
    if (error) {
      console.error('[Admin] 更新助手失败:', error)
      return NextResponse.json(
    ...
    console.error('[Admin] 更新助手错误:', error)
    return NextResponse.json(
      { error: '服务器错误' },
      { status: 500 }
    )
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      console.error('[Admin] 删除助手失败:', error)
      return NextResponse.json(
    ...
    console.error('[Admin] 删除助手错误:', error)
    return NextResponse.json(
      { error: '服务器错误' },
      { status: 500 }
    )
